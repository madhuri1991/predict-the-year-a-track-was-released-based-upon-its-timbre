...
dim(track_metadata_tbl)# track_metadata_tbl has been pre-defined
track_metadata_tbl
...
track_metadata_tbl %>%
  ...
  distinct(artist_name) #Unique, distinct, and non-duplicated all mean the same thing.
...
#mutate() and summarize() are commonly paired with group_by().
# Write SQL query
...
# track_metadata_tbl, afinn_sentiments_tbl have been pre-defined
track_metadata_tbl
...
#It looks the like random forest model is a little better, though both models struggle with old songs.